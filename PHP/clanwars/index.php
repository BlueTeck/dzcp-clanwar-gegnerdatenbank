...
default:
  ...
  while($get = _fetch($qry))
  {
    ...
    $qrym = db("SELECT s1.id,s1.datum,s1.cid,s1.xonx,s1.liga,s1.punkte,s1.gpunkte,s1.maps,s1.serverip,
                       s1.servername,s1.serverpwd,s1.bericht,s1.squad_id,s1.gametype,s2.icon,s2.name
                FROM ".$db['cw']." AS s1
...
case 'showall';
  if(isset($_GET['page'])) $page = $_GET['page'];
	...
	$qry = db("SELECT s1.id,s1.datum,s1.cid,s1.xonx,s1.liga,s1.punkte,s1.gpunkte,s1.maps,s1.serverip,
                    s1.servername,s1.serverpwd,s1.bericht,s1.squad_id,s1.gametype,s2.icon,s2.name
             FROM ".$db['cw']." AS s1
             LEFT JOIN ".$db['squads']." AS s2 ON s1.squad_id = s2.id
  ...
  if(_rows($qry))
  {
    ...
    {
//Clans Mod Start
$clandetailssql = db("SELECT clantag, gegner, url, country FROM ".$sql_prefix."clans WHERE id LIKE ".$get['cid']);
		$clans = _fetch($clandetailssql);
      
	  $flagge = flag($clans['country']);
      $gegner = show(_cw_details_gegner, array("gegner" => re(cut($clans['clantag']." - ".$gclans['gegner'], $lcwgegner)),
                                               "url" => '../clans/?action=show&amp;id='.$get['cid']));		
//Clans Mod Ende		
      $img = squad($get['icon']);
...
break;
case 'details';
  $qry = db("SELECT s1.id,s1.datum,s1.cid,s1.xonx,s1.liga,s1.punkte,s1.gpunkte,s1.maps,s1.serverip,s1.servername,
                    s1.serverpwd,s1.bericht,s1.squad_id,s1.gametype,s1.lineup,s1.glineup,s1.matchadmins,s2.icon,s2.name,s2.game
           FROM ".$db['cw']." AS s1
           LEFT JOIN ".$db['squads']." AS s2 ON s1.squad_id = s2.id
...
$clandetailssql = db("SELECT clantag, gegner, url, country FROM ".$sql_prefix."clans WHERE id LIKE ".$get['cid']);
  ...
  $flagge = flag($clans['country']);
  $gegner = show(_cw_details_gegner_blank, array("gegner" => re($clans['clantag']." - ".$clans['gegner']),
                                                 "url" => '../clans/?action=show&amp;id='.$get['cid']));
//Clans Mod Ende												 
  ...
  foreach($picformat AS $end)
  {
       if(file_exists(basePath.'/inc/images/clanwars/'.$get['cid'].'_logo.'.$end)) $logo_gegner = $get['cid'].'_logo.'.$end;
