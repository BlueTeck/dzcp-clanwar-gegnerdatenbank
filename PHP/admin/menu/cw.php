...
ORDER BY game");
    ...
    //Clans Mod (Gegner is empty now / comment code)
    ...
    if (empty($_POST['t'])){
        ...
        //Clans Mod (add cid to DB-Insert)
    ...
    if (empty($_POST['t'])) {
        ...
        //Clans Mod (add cid to DB-Update)
        $qry = db("UPDATE " . $db['cw'] . "
                    SET " . $xonx . "
                    " . $kid . "
                    `datum` = '" . ((int) $datum) . "',
...
ORDER BY game ASC");
    ...
    while ($get = _fetch($qry)) {
        $top = empty($get['top']) ? '<a href="?admin=cw&amp;do=top&amp;set=1&amp;id=' . $get['id'] . '"><img src="../inc/images/no.gif" alt="" title="' . _cw_admin_top_set . '" /></a>' : '<a href="?admin=cw&amp;do=top&amp;set=0&amp;id=' . $get['id'] . '"><img src="../inc/images/yes.gif" alt="" title="' . _cw_admin_top_unset . '" /></a>';
